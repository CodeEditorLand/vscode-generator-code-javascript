...
module.exports = generators.Base.extend({
	constructor: function () {
		...
		this.argument("codeExec", {
			type: String,
			defaults: "code-insiders",
			optional: true,
		});
	...
	_checkConfigJson() {
		...
		this.ctx.jsConfigJson = this.fs.readJSON(
			this.destinationPath("jsconfig.json"),
		);
		this.ctx.tsConfigJson = this.fs.readJSON(
	...
	_checkInstalledExtensions() {
		...
		cp.exec(`${this.codeExec} --list-extensions`, (err, stdout, stderr) => {
			if (!err) {
				this.ctx.installedExtensions = new Set(
					stdout.trim().split(/[\r\n]/),
				);
			}
			resolve();
	...
	prompting() {
		const prompts = [
			...
			{
				when: () =>
					this.ctx.tsConfigJson &&
					(!this.ctx.tsConfigJson.compilerOptions ||
						!this.ctx.tsConfigJson.compilerOptions.allowJs),
				type: "confirm",
				name: "setAllowJs",
				message:
					"Should I adjust 'tsconfig.json' to allow for JavaScript files (strongly recommended)?",
			},
			{
				when: (answers) =>
					!answers.setAllowJs &&
				...
				name: "setAllowJs",
				message:
					"Sure about that? The presence of a 'tsconfig.json'-file shadows a 'jsconfig.json'-file and without the 'allowJs'-flag there will be no support for JavaScript. Should I add the 'allowJs'-flag?",
			},
			{
				...
				name: "acquireTypes",
				message:
					"Install type-definition files (.d.ts) and adjust 'package.json'?",
			},
			{
				when: () =>
					this.ctx.installedExtensions &&
					!this.ctx.installedExtensions.has("eg2.vscode-npm-script"),
				type: "confirm",
			...
			},
			{
				when: () =>
					this.ctx.installedExtensions &&
					!this.ctx.installedExtensions.has("dbaeumer.vscode-eslint"),
				type: "confirm",
				name: "installEsLint",
	...
	writing() {
		if (this.answers.createJsConfig) {
			...
			fs.writeFileSync(
				this.destinationPath("tsconfig.json"),
				JSON.stringify(this.ctx.tsConfigJson, undefined, 4),
			);
		}
		...
		if (this.answers.acquireTypes) {
			fs.writeFileSync(
				...
				JSON.stringify(this.ctx.newPackageJson, undefined, 4),
			);
	...
	install() {
		...
		if (this.answers.installNpmScriptRunner) {
			this.spawnCommand(this.codeExec, [
				"--install-extension",
